...
type Props = {
  ...
  max?: string;
  /** Имя поля Start для валидации end > start */
  startName?: string;
  /** getValues из react-hook-form (используется для сравнения start/end) */
  getValues?: UseFormGetValues<any>;
};

export const RHFTimeEnd: React.FC<Props> = ({
  name,
  id,
  className,
  disabled,
  ...
  return (
    <Controller
      ...
      rules={{
        ...
        validate: (v: string) => {
          if (startName) {
            const start = effectiveGetValues(startName);
            if (start && timeToMinutes(v) <= timeToMinutes(start)) {
